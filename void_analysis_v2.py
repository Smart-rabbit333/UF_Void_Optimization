...
b1 = ax1.bar(xp-w/2, maes, w, color='#888', label='MAE')

# 변경됨: 좌측 Y축 라벨 'MAE'
ax1.set_ylabel('MAE (%)')
ax1.set_xticks(xp); ax1.set_xticklabels(names)
ax1.set_ylim(0, max(maes)*1.35) # 범례를 내부에 넣기 위해 윗 공간을 충분히 확보

ax2 = ax1.twinx(); ax2.grid(False)
b2 = ax2.bar(xp+w/2, r2s, w, color='#d6604d', label='R$^2$')
...
# 변경됨: pad 옵션 제거 및 범례(legend) 박스 안으로 이동
ax1.set_title('Model comparison — LOOCV (n = 45)')
h1,l1 = ax1.get_legend_handles_labels()
h2,l2 = ax2.get_legend_handles_labels()

# bbox_to_anchor 제거, loc을 'upper center'로 지정하고 배경을 하얗게(frameon=True) 지정하여 가독성 향상
